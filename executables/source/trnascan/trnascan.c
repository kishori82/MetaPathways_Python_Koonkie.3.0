...
-F <outfasta>   : write predicted tRNA genes in fasta format<outfile>\n\
-i <infile>    : reads the sequences from <infile>\n\
-D <TPCsignal> : reads the Dsignal from <TPCsignal>\n\
-T <Dsignal>   : reads the Tsignal from <Dsignal>\n\
-f <integer>   : start sequence numbering at <integer> (def=1)\n\
...
following_search(long int pos,    /* first position of the found T-Psi-C signal */ 
         ...
         FILE *fpf,  /* pointer to the output file for  predited tRNA genes */ 
		 FILE *fpverb, 
		 char *name, 
		 ...
		 long int sqoffset,   /* offset nucleotide numbering by this much (set with -i param) */
          int *npred_seq  /* number of sequences predicted with tRNA */
		 );
...
void printresult(FILE *fpo,     /* output file pointer */
        FILE *fpf,  /* pointer to the output file for  predited tRNA genes */ 
...
int main(int argc, char **argv)
{
  ...
  int npred=0; /* Total number of tRNA genes predicted in the sequences of 
		  the input file */
  int nseq=0; /* Number of sequences tested (number of sequences of the input file) */
  int npred_seq=0; /* Number of sequences with tRNA genes in them */
  ...
  char *seqfile,   /* name of input sequence file  */
    *outfile,   /* name of output file (if not sent to stdout) */
     *Dsignalfile, /* name of the D signal file */
     *Tsignalfile, /* name of the T-to-C  signal file */
    *verbfile,  /* name of file for verbose output */
    *outfasta;  /* name of the file for predicted tRNA genes*/
...
while ((optc = getopt(argc, argv, "csraho:F:v:f:i:T:D:")) != -1)
  switch (optc) {
  ...
  case 'F': outfasta   = optarg;  break;
  case 'i': seqfile   = optarg;  break;
  case 'T': Tsignalfile   = optarg;  break;
  case 'D': Dsignalfile   = optarg;  break;
  case 'v': verbfile  = optarg;  break;
  case 'f': sqoffset = atoi(optarg)-1; break;
...
if ((fpf=fopen(outfasta,"w")) == NULL) {
  fprintf(stderr,"tRNAscan1.4: FATAL: Cannot open the fasta output file %s\n",outfasta);
  exit(1);
}
...
if (((fpcons1=fopen(Tsignalfile,"r")) == NULL) &&
...
if (((fpcons2=fopen(Dsignalfile,"r")) == NULL) &&
...
#ifdef VERBOSE
	  fprintf(fpverb,"--- Moving TPC window (pos=%ld)\n",pos);
#endif 
...
/*the general score SG (score) is incremented by 1, if the number of 
  ...
  MODIFY the following line */
	  ...
	  score++;
#ifdef VERBOSE
	  fprintf(fpverb,"+ TPC invariant bp: %ld.  SG++\n",ninv); }
	else  { 
	  fprintf(fpverb,"X TPC invariant bp: %ld.  NO SG++\n",ninv); 	  
#endif 
	}
...
/* If the number of base-pairings (ncomp) equal 5, SG (score) incremented 
   ...
   the increment of SG, MODIFY the value '5' in the following line */
          ...
          if (ncomp >= ps.tpc_incsg) {
            score++;
#ifdef VERBOSE
 	    fprintf(fpverb,"+ TPC parings (%ld) add to SG=%ld\n",ncomp,score); }
	  else  {
 	    fprintf(fpverb,"X TPC parings (%ld) NO add to SG=%ld\n",ncomp,score); 
#endif 
	  }
...
/* Change in the threshold to retain a stem as a potential T-Psi-C arm,
   MODIFY the value '4' in the following line */
	    ...
	    fprintf(fpverb,"X TPC parings retain loop: %ld\n",ncomp); }
	  else {
	    fprintf(fpverb,"+ TPC parings retain loop: %ld\n",ncomp); 
#else
          if (ncomp >= ps.tpc_keep)  {
#endif 
...
/* Search for the D signal */
              ...
              if(!readsignal(ptr1,table_inv2,&lsig2,&ktot2,weight2,
                ...
                {
		  fprintf(fpverb,"X D invariant bp: %ld\n",ninv); }
	      else 
		{
		  fprintf(fpverb,"+ D invariant bp: %ld\n",ninv); 	     
#else
              if(readsignal(ptr1,table_inv2,&lsig2,&ktot2,weight2,
                 table_cons2,&ninv,threshold_inv))
                {
...
/* Number of invariant bases found in the windowed sequence equal to the
   ...
   line */
		  ...
		  if (ninv >= ktot2) {
		    score++;
#ifdef VERBOSE
		    fprintf(fpverb,"+ D invariant (%ld) inc SG\n",ninv); }
		  else {
		    fprintf(fpverb,"X D invariant (%ld) NO add to SG\n",ninv);
#endif 
		  }
...
/* If stems with 3 base-pairings are found for the D arm, the following 
   ...
   The 2 base-pairings stems are discarded */
                    ...
                    if (ntab[h][0] == 3)
                      ...
                      sequence,seqlen,fpo,fpf,fpverb,name,score,match2,&ntrna,
                      &npred,&match,sqoffset,&npred_seq);
                      }
                    }
...
/* The sequence is complemented and the algorithm is applied on the 
   ...
   // fprintf(fpo,"number of sequences with predicted tRNA = %d\n", npred_seq);
...
void following_search(long int pos,    /* first position of the found T-Psi-C signal */ 
		 long int pos1,   /* first position of the found D signal */
		 char *ptr1,      /* pointer to the first position of the D signal */
		 char *ptr3,      /* ptr3=ptr1+2  pointer to first position of the D arm  */
		 int lpair, 
		 ...
		 int *match,
		 long int sqoffset,   /* offset nucleotide numbering by this much (set with -i param) */
		 int *npred_seq   /* number of sequences with tRNA genes found */
		 )
...
if (ncomp >= ps.aa_incsg) {
  score1++;
#ifdef VERBOSE
  fprintf(fpverb,"+ AA arm found %ld base pairings\n",ncomp); }
else {
  fprintf(fpverb,"X AA arm found %ld base pairings\n",ncomp); 
#endif 
}
...
/* If SG >= 5, the windowed sequence is retained as a potential tRNA gene.
   The tRNA gene predicted at that level is without intron */
	...
	fprintf(fpverb,"X Under SG threshold: %ld\n",score1); }
      else  {
	fprintf(fpverb,"+ Over SG threshold: %ld\n",score1); 
#else
      if (score1 >= ps.sg_cutoff)  {
#endif 
...
/* The results are printed in the output file */
        ...
        printresult(fpo,fpf,fpverb,name,pos1,pos,lpair,lpair1,lpair2,nloop,ntrna,
...
/* Test for the presence of the residue T at the position preceding the
   ...
   incremented by 1.*/
            ...
            if ((*(ptr4+6)) == 't') {
              score2++;
#ifdef VERBOSE
	      fprintf(fpverb,"+ Found invariant T. SG= %ld\n",score2); }
      else  {
	      fprintf(fpverb,"X Did NOT find invariant T. SG= %ld\n",score2); 
#endif 
	    }
...
/* The results are printed in the output file */
              ...
              printresult(fpo,fpf, fpverb,name,pos1,pos,lpair,lpair1,lpair2,nloop,
...
//  fprintf(fpo,"D signal\tTpsyC signal\t");
 ...
 // fprintf(fpo,"tRNA predict as a tRNA\t");
 // fprintf(fpo,"potential intron between positions\t");
//  fprintf(fpo,"number of base pairing in the anticodon stem\n"); 
}
...
*/
  if (strcmp(type_trna,"Ind") != 0) {
    fprintf(fpo,"%s\t%s\t", type_trna, anticodon);
  }
  else {
    fprintf(fpo," \t \t");
  } 
...
void set_search_params (Param_set_type *ps, int params)
  {
